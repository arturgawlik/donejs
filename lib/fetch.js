/**
 * How `fetch` should work:
 * 1. Promise returned from `fetch` should resolve as soon as server starts to send body.
 * 2. Promise returned from `text`/`json` should resolve as soon as whole body was send.
 */

import doneURL from 'done:url.js';
import doneHttpParser, { Kind } from 'done:http-parser.js';
...
class Response {
    ...
    #httpResponseParser = new doneHttpParser(Kind.RESPONSE);
